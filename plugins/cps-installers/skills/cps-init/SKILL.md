...
> **Rev 4 (2026-04-11):** Ported into the `cps-installers` plugin. Source and distribution moved from `/mnt/.claude/skills/cps-init/` to `plugins/cps-installers/skills/cps-init/` in the CPS source-of-truth repo. Install path is now `/plugin install cps-installers@cps --scope project` instead of a global `.skill` bundle. Also fixed the fetch URL: previous revs referenced the retired `claude-project-system` repo name; rev 4 uses `cowork-project-system` for all GitHub fetches.
>
> **Rev 3 (2026-04-10):** No longer bundles `cps_scaffold.py`. Fetches it from GitHub at runtime, same pattern as the (now retired) `cps-patcher`'s catalog fetch. Removed the "rebundle cps-init.skill on scaffold edit" house rule — scaffold edits propagate via `git push` to main.
>
...
This skill is part of the `cps-installers` plugin. The plugin ships SKILL.md only. The scaffolder is fetched at runtime from:

```
https://raw.githubusercontent.com/Huesdon/cowork-project-system/main/Reference/cps_scaffold.py
```

The `.ps1`/`.cmd` Windows standalone fallbacks are no longer bundled. Users who want to scaffold manually outside of Cowork can download them directly from the GitHub repo.
...
## Contract for programmatic callers (e.g. `cps-setup` Step 5)
...
5. **Network requirement:** this skill requires outbound HTTPS to `raw.githubusercontent.com`. Offline environments will fail at Step 4. There is no bundled fallback by design.
