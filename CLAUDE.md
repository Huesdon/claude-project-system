...
> **Last Updated:** 2026-04-10 (rev 6 — §0 added patcher catalog house rule)
...
- **Patch catalog entry on new patchable feature.** Any session that adds a **structurally new** scaffolded artifact (new dirs, new stub files, new CLAUDE.md section blocks, new config keys) MUST add a corresponding patch entry to `Skills/cps-patcher/references/patch-catalog.md` and rebundle `cps-patcher.skill` before close. Downstream projects silently miss features if the catalog is not updated same-session. **Does NOT apply to:** bugfixes that correct existing template content, rev variable corrections, or skill rewrites that don't change what gets deployed to downstream projects — those require no catalog entry.
